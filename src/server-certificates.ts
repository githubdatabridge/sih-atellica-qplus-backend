...
const getServerCertificatePath = (certFileName: string): string => {
    ...
    const settings = {
        SERVER_CERT_PATH: path.resolve(
            configService.get(ENV_PARAMS.SERVER_CERT_PATH)
        ),
    };
...
const getServerCertificates = (): ServerCertificates => {
    ...
    certs = {
        ...
        cert: readServerCert(
            configService.get(ENV_PARAMS.SERVER_CERT_FILE_NAME)
        ),
    };
...
const getTokenCertificates = (): TokenCertificates<string> => {
    const serverCertPath = path.resolve(`${__dirname}/certificates/data`);
